...
.TIPS
    One can query the subscription id using the azure cli and passing it in a variable as: SubscriptionId="$(az account list --query [].id -o tsv)"
#>
...
# Helper Functions ------------------------------------------------------> can't find the file, maybe it's in the external SSD?
. ..\..\Common\PS\Util.ps1
